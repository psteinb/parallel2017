...
\begin{tikzpicture}[every node/.style={color=white}]
  ...
  \node [below=of pcie, text width=4cm, align=center] (stdvec) {\texttt{hc::copy}, \texttt{hc::async\_copy}, \texttt{std::future}, \texttt{std::when\_all}, \dots{}};

  \node [below=5mm of device, align=center] (stdvec) {\texttt{std::for\_each(vendor::device\_policy, ...}};

  \node [below=5mm of host, align=center] (stdvec) {\texttt{std::for\_each(std::par, ...)}};
