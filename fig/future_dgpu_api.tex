...
\begin{tikzpicture}[every node/.style={color=white}]
  ...
  \node [above=5mm of device_memory,text width=3cm, align=center] (hcarr) {\texttt{vendor::array} \texttt{vendor::array\_view}};
  
  \node [below=of pcie, text width=4cm, align=center] (stdvec) {\texttt{vendor::copy}, \texttt{vendor::async\_copy}, \texttt{std::future}, \texttt{std::when\_all}, \dots{}};

  \node [below=5mm of device.south west, align=left,anchor=west, text width=7.5cm] (dforeach) {\texttt{std::for\_each(vendor::par, ...)}};

  \node [below=1mm of dforeach, align=left,text width=7.5cm] (dacc) {\texttt{std::accumulate(vendor::par, ...)}};

  \node [below=5mm of host.south east, align=center, anchor=east] (stdvec) {\texttt{std::for\_each(std::par, ...)}};
